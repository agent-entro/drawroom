...
exports_.post('/', async (c) => {
  ...
  if (!body?.roomId || !body.participantId || !body.format || !body.dataBase64) {
    return c.json({ error: 'roomId, participantId, format, and dataBase64 are required' }, 400);
  }
  ...
  try {
    ...
    const dirPath = join(EXPORT_DIR, body.roomId);
    ...
    const relPath = join(body.roomId, filename);

    await sql`
      INSERT INTO exports (id, room_id, format, file_path, file_size_bytes, requested_by)
      VALUES (${id}, ${body.roomId}, ${body.format}, ${relPath}, ${fileStat.size}, ${body.participantId})
    `;
